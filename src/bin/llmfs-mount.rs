...
enum Command {
    ...
    Start {
        ...
        /// Remaining arguments passed to the backend (llmfs-mount-nfs or llmfs-mount-fuse)
...
fn main() {
    ...
    match cli.command {
        Command::Stop { mount_point } => match llmfs_mount::daemon::stop_daemon(&mount_point) {
            Ok(()) => {}
            Err(e) => {
                eprintln!("Error: {e}");
                std::process::exit(1);
        ...
        Command::Status => {
            let daemons = llmfs_mount::daemon::list_daemons();
            if daemons.is_empty() {
        ...
        Command::Start { fuse, args } => {
            let backend = if fuse { "llmfs-mount-fuse" } else { "llmfs-mount-nfs" };
            ...
            let mount_point = match mount_point {
                ...
                None => {
                    ...
                    eprintln!(
                        "Usage: llmfs-mount start [--fuse] [backend-options...] <subcommand> <source> <mount-point>"
                    );
                    std::process::exit(1);
                }
            };
            ...
            let source_label = args
                .iter()
                .position(|a| a == "bucket" || a == "repo" || a == "gitea" || a == "github" || a == "gitlab")
                .and_then(|i| args.get(i + 1).map(|id| format!("{} {}", args[i], id)))
                .unwrap_or_else(|| "unknown".to_string());

            // Daemonize: fork, setsid, redirect logs, write PID file.
            let guard = match llmfs_mount::daemon::daemonize(&mount_point, &source_label) {
                Ok(g) => g,
                Err(e) => {
                    eprintln!("Error: {e}");
                    std::process::exit(1);
