...
/// State directory for daemon files (~/.llmfs-mount/).
...
/// Check if a PID belongs to a live llmfs-mount daemon process.
...
fn is_our_daemon(pid: i32) -> bool {
    ...
    #[cfg(target_os = "linux")]
    {
        if let Ok(exe) = std::fs::read_link(format!("/proc/{pid}/exe")) {
            ...
            return name.starts_with("llmfs-mount");
        }
    }
...
impl DaemonGuard {
    ...
    /// Create a DaemonGuard from env vars set by the `llmfs-mount` daemon.
    /// Returns None if not running as a daemon subprocess.
    pub fn from_env() -> Option<Self> {
        let fd: i32 = std::env::var("LLMFS_MOUNT_DAEMON_FD").ok()?.parse().ok()?;
        let pid_file = std::env::var("LLMFS_MOUNT_DAEMON_PID_FILE").ok().map(PathBuf::from)?;
        // SAFETY: called at startup before any threads are spawned.
        unsafe {
            std::env::remove_var("LLMFS_MOUNT_DAEMON_FD");
            std::env::remove_var("LLMFS_MOUNT_DAEMON_PID_FILE");
        }
        Some(Self {
            write_fd: fd,
            pid_file,
    ...
    if let Some(existing) = read_pid_file(&pid_file) {
        if is_daemon_alive(&existing, &canonical) {
            return Err(std::io::Error::new(
                ...
                format!(
                    "daemon already running (pid={}) for {:?}. Stop it first with: llmfs-mount stop {}",
                    existing.pid,
                    mount_point,
                    mount_point.display()
                ),
    ...
    match unsafe { libc::fork() } {
        ...
        child_pid => {
            ...
            if n == 1 && buf[0] == b'R' {
                ...
                eprintln!("  stop: llmfs-mount stop {}", canonical.display());
                std::process::exit(0);
            } else {
                eprintln!(" failed");
