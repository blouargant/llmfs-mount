...
fn make_clients(backend: &str) -> (Client, Client) {
    let user_agent = format!("llmfs-mount/{}; fs/{}", env!("CARGO_PKG_VERSION"), backend);
...
impl HubApiClient {
    ...
    pub async fn from_source(
        ...
        let (source, last_modified) = match source {
            ...
            SourceKind::GiteaRepo { .. } | SourceKind::GitHubRepo { .. } | SourceKind::GitLabRepo { .. } => {
                unreachable!("external providers use their own client, not HubApiClient")
            }
        };
...
mod tests {
    ...
    fn test_token_dir() -> PathBuf {
        let dir = std::env::temp_dir().join(format!("llmfs-mount-test-{}", std::process::id()));
