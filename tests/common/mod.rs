...
pub async fn setup_bucket(test_name: &str) -> Option<(String, String, Arc<llmfs_mount::hub_api::HubApiClient>)> {
    ...
    let bucket_id = format!("{}/llmfs-mount-{}-{}", username, test_name, std::process::id());

    create_bucket(&ep, &token, &bucket_id).await;
    eprintln!("Created bucket: {}", bucket_id);

    let hub = llmfs_mount::hub_api::HubApiClient::new(&ep, Some(&token), &bucket_id, "test");
...
pub async fn setup_bucket_with_file(
    ...
    filename: &str,
    content: &[u8],
) -> Option<(String, String, Arc<llmfs_mount::hub_api::HubApiClient>)> {
    let (token, bucket_id, hub) = setup_bucket(test_name).await?;
    let write_config = build_write_config(&hub).await;

    let tmp_dir = std::env::temp_dir().join(format!("llmfs-mount-{}-setup", test_name));
    ...
    hub.batch_operations(&[llmfs_mount::hub_api::BatchOp::AddFile {
...
pub async fn build_write_config(hub: &Arc<llmfs_mount::hub_api::HubApiClient>) -> Arc<TranslatorConfig> {
...
/// Spawn llmfs-mount-fuse as a child process, wait until the mountpoint is live.
...
pub fn mount_bucket(bucket_id: &str, mount_point: &str, cache_dir: &str, extra_args: &[&str]) -> Child {
    ...
    let child = Command::new(binary)
        .env(
            "RUST_LOG",
            std::env::var("RUST_LOG").unwrap_or_else(|_| "llmfs_mount=warn".to_string()),
        )
        .args([
            "--hf-token",
            &token,
        ...
        .spawn()
        .expect("Failed to spawn llmfs-mount-fuse");
...
/// Spawn llmfs-mount-fuse to mount a repo as read-only, wait until the mountpoint is live.
...
/// Spawn llmfs-mount-nfs to mount a bucket via NFS.
pub fn mount_bucket_nfs(bucket_id: &str, mount_point: &str, cache_dir: &str, extra_args: &[&str]) -> Child {
    ...
    let binary = std::env::current_exe()
        ...
        .parent()
        .unwrap()
        .join("llmfs-mount-nfs");

    eprintln!("Mounting NFS with binary: {:?}", binary);

    if !binary.exists() {
        panic!("llmfs-mount-nfs binary not found, run cargo build --release first");
    }
    ...
    let child = Command::new(binary)
        ...
        .args(["bucket", bucket_id, mount_point])
        .spawn()
        .expect("Failed to spawn llmfs-mount-nfs");
...
/// Unmount FUSE and wait for llmfs-mount to exit. Waits up to `graceful_secs`
...
/// Unmount NFS and wait for llmfs-mount to exit.
...
fn unmount_with(mount_point: &str, mut child: Child, graceful_secs: u64, cmd: &[&str]) {
    ...
    for _ in 0..graceful_secs {
        if let Ok(Some(status)) = child.try_wait() {
            eprintln!("llmfs-mount exited: {}", status);
            return;
        }
    ...
    child.kill().ok();
    match child.wait() {
        Ok(status) => eprintln!("llmfs-mount killed: {}", status),
