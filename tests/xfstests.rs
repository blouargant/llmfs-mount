...
//! llmfs-mount with --advanced-writes. Patches xfstests to work with FUSE:
...
fn create_mount_wrapper(binary: &std::path::Path) {
    ...
    let wrapper = format!(
        "#!/bin/bash\nMOUNTPOINT=\"$1\"\nmkdir -p \"$MOUNTPOINT\"\n\
         export RUST_LOG=${{RUST_LOG:-llmfs_mount=warn}}\n\
    ...
    std::fs::write("/usr/local/bin/llmfs-mount", &wrapper).ok();
    // Try with sudo if direct write fails
    if !std::path::Path::new("/usr/local/bin/llmfs-mount").exists() {
        Command::new("sudo")
            .args(["tee", "/usr/local/bin/llmfs-mount"])
    ...
    Command::new("sudo")
        .args(["chmod", "+x", "/usr/local/bin/llmfs-mount"])
        .status()
        .ok();
}
...
fn write_config(test_dir: &str, scratch_dir: &str) {
    let config = format!(
        "export FSTYP=fuse\n\
         export TEST_DEV=llmfs-mount\n\
         export TEST_DIR={}\n\
         export SCRATCH_DEV=llmfs-mount\n\
         export SCRATCH_MNT={}\n",
